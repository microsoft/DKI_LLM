...
if script_args.reward_score_normaliaztion:
    for epoch_id in range(script_args.resume_epoch,script_args.epochs):
        if script_args.resume_epoch == epoch_id:
            resume_start_step = script_args.resume_steps
        else:
            resume_start_step = 0
        #start = script_args.resume_steps if script_args.resume_epoch==epoch_id else 0
        for step, batch in enumerate(tqdm(ppo_trainer.dataloader),start=0):
