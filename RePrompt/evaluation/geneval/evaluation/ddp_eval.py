...
def parse_args():
    ...
    parser.add_argument('--fix_nccl', type=bool, default=False)
    parser.add_argument("--model-config", type=str, default=None)
    parser.add_argument("--model-path", type=str, default="object")
    # Other arguments
...
def main(args):
    ...
    if not args.fix_nccl and os.path.exists(args.outfile):
    ...
    if args.fix_nccl:
        ...
        with open(args.outfile, 'r') as file:
            for line in file:
                ...
                exist_filename.append(json_obj['filename'])
    # Save results
    ...
    full_results = []
    subfolder_set = [name for name in os.listdir(args.imagedir) if os.path.isdir(os.path.join(args.imagedir, name))]
    subfolder_set = sorted(subfolder_set, key=lambda x: int(x))
    subfolder_set = subfolder_set
    dist.barrier()

    global_n_samples = world_size
    total_subfolder_set = int(math.ceil(len(subfolder_set) / global_n_samples) * global_n_samples)
    new_subfolder_set = subfolder_set + [subfolder_set[0]] * (total_subfolder_set - len(subfolder_set))
    per_gpu_subfolder_set = new_subfolder_set[rank:total_subfolder_set:global_n_samples]

    for index, subfolder in tqdm(enumerate(per_gpu_subfolder_set), total=len(per_gpu_subfolder_set)):
        if index * global_n_samples + rank > len(subfolder_set) - 1:
            break
        ...
        imagepath = os.path.join(folderpath, 'final.png')
        # if not os.path.isfile(imagepath) or not re.match(r"\d+\.png", 'final.png'):
        #     import ipdb;ipdb.set_trace()
        #     continue
    ...
    #
    # # Only the main process (rank 0) will save the results
    # if rank == 0:
    ...
    #         pd.DataFrame(all_results).to_json(fp, orient="records", lines=True)
