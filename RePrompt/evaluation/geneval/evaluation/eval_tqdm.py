...
def main(args):
    ...
    for subfolder in tqdm(subfolders, desc="Evaluating folders"):
        folderpath = os.path.join(args.imagedir, subfolder)
        with open(os.path.join(folderpath, "metadata.jsonl")) as fp:
            metadata = json.load(fp)
        # Evaluate each image
        image_files = [f for f in os.listdir(os.path.join(folderpath, "samples")) if re.match(r"\d+\.png", f)]
        for imagename in tqdm(image_files, desc=f"Evaluating images in folder {subfolder}"):
