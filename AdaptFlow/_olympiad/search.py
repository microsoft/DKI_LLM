...
from olympiad_prompt import get_init_archive, get_prompt, get_meta_prompt, get_adaptation_prompt, get_reflexion_prompt, get_fail_reflexion_prompt

from utils import load_all_examples, is_correct, random_id, bootstrap_confidence_interval, extract_median, judge
...
def evaluate_forward_fn_key(args, forward_str, key, file_dir):
    ...
    acc_list = []
    fail_examples_list = []
    results = [None] * len(task_queue)  # 初始化结果列表，按顺序填充

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(agentSystem.forward, task): idx for idx, task in enumerate(task_queue)}
        
        for future in tqdm(as_completed(futures), total=len(task_queue), desc="Processing Tasks"):
            idx = futures[future]
            try:
    ...
    for idx, res in enumerate(results):
        try:
            ...
            correct = judge(correct_answer, extracted_answer)
        except Exception as e:
            acc_list.append(0)
...
def process_key_evaluation(args, key, sol, adaptation_archive, acc_list_dic, adaptation_file_path):
    ...
    try:
        if sol['generation'] == "initial":
            ...
            task_dsc = "\n".join([f"##The task case {i+1} is following:\n{task['question']}" for i, task in enumerate(task_dsc)])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--expr_name', type=str, default="olympiad_gpt4omini_results")
    parser.add_argument('--n_inner_loop', type=int, default=4)
    parser.add_argument('--n_outer_loop', type=int, default=2)
    parser.add_argument('--debug_max', type=int, default=2)
    parser.add_argument('--version', type=str, default="v1")
    parser.add_argument('--train_dir_path', type=str, default="dataset_v3/olympiad/validate")
    parser.add_argument('--test_dir_path', type=str, default="dataset_v3/olympiad/test")
    # test_math_486
