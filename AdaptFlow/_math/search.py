...
from math_prompt import get_init_archive, get_prompt, get_meta_prompt, get_adaptation_prompt, get_reflexion_prompt, get_fail_reflexion_prompt

from utils import load_all_examples, score_math, random_id, cal_acc, extract_accuracy
...
class LLMAgentBase():
    ...
    def generate_prompt(self, input_infos, instruction) -> str:
        ...
        # an example: Here is a solution example: We are given that $f(x) = 3x + 5$. We need to find $f(2)$, which means we substitute $x = 2$ into the expression: $f(2) = 3 \cdot 2 + 5 = 6 + 5 = \\boxed{11}$
        ...
        prompt = instruction + input_infos_text
...
def search(args):
    ...
    for solution in archive["all"]:
        ...
        for key in key_list:
            if key == "all":
                continue
            if key in acc_list_dic and solution['name'] in acc_list_dic[key]:
...
def evaluate_forward_fn_key(args, forward_str, key, file_dir):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(agentSystem.forward, task): idx
            for idx, task in enumerate(task_queue)
        }

        for future in tqdm(as_completed(futures), total=len(task_queue), desc="Processing"):
            idx = futures[future]
            try:
    ...
    for idx, res in enumerate(results):
        try:
            ...
            correct_answer = solutions[idx]
            correct = score_math(correct_answer, extracted_answer)
        except Exception as e:
            acc_list.append(0)
            # 添加字符串，要包含信息problem, solution, extracted_answer
            fail_examples_list.append(f"problem: {problems[idx]}, correct_answer: {solutions[idx]}, extracted_answer: {extracted_answer}")
            continue
        acc_list.append(1 if correct else 0)
        if not correct:
            fail_examples_list.append(f"problem: {problems[idx]}, correct_answer: {solutions[idx]}, extracted_answer: {extracted_answer}")
    print(f"acc: {cal_acc(acc_list)}")
...
def process_key_inner_loop(args, key, archive, dir_path, n_outer, out_start, inner_start_dict):
    ...
    for sol in archive:
        if 'fitness' in sol:
            try:
                fitness = extract_accuracy(sol['fitness'])
    ...
    for n_inner in range(inner_start, inner_start + args.n_inner_loop):
        ...
        if extract_accuracy(fitness_str) > max_fitness:
            break
...
def process_key_evaluation(args, key, sol, adaptation_archive, acc_list_dic, adaptation_file_path):
    ...
    try:
        if sol['generation'] == "initial":
            ...
            # task_dsc = "\n".join([f"##The task case {i+1} is following:\n{task['problem']}" for i, task in enumerate(task_dsc)])
            task_dsc = "\n".join([("##The task case" + str(i + 1) + "is following:\n" + task['problem']) for i, task in enumerate(task_dsc)])
            ...
            acc_list = []
            for _ in range(args.debug_max):
                acc_list, _ = evaluate_forward_fn_key(args, next_solution["code"], key, args.test_dir_path)
                if np.mean(acc_list) < 0.01:
                    next_solution = get_json_response_from_gpt_reflect(msg_list, args.update_model)
                else:
                    break
            acc_list_dic[key] = acc_list
            fitness_str = cal_acc(acc_list_dic[key])
        ...
        acc_list_dic[key] = []
        next_solution = {}
    return acc_list_dic[key], next_solution


def evaluate(args):
    file_path = os.path.join(args.save_dir, f"{args.expr_name}_run_archive_{args.version}", "all.json")
    if args.test_adaption:
        eval_file_path = os.path.join(args.save_dir, f"{args.expr_name}_run_archive_{args.version}_evaluate.json")
    else:
        eval_file_path = os.path.join(args.save_dir, f"{args.expr_name}_run_archive_{args.version}_evaluate_wo_adaption.json")
    adaptation_file_path = os.path.join(args.save_dir, f"{args.expr_name}_run_archive_{args.version}_evaluate_adaptation.json")
    ...
    for sol in archive:
        ...
        if args.test_adaption:
            with ThreadPoolExecutor(max_workers=len(key_list)) as executor:
                ...
                for future in as_completed(futures):
                    key = futures[future]
                    try:
                        acc_list_dic[key], next_solution = future.result()
                    except Exception as e:
            ...
            # for key in key_list:
            #     acc_list,_ = evaluate_forward_fn_key(args, sol["code"], key, args.test_dir_path)
            #     acc_list_dic[key] = acc_list

            with ThreadPoolExecutor(max_workers=len(key_list)) as executor:
                futures = {
                    executor.submit(evaluate_forward_fn_key, args, sol["code"], key, args.test_dir_path): key 
                    for key in key_list
                }
                ...
                for future in as_completed(futures):
                    key = futures[future]
                    try:
                        acc_list_dic[key], _ = future.result()
                    except Exception as e:
                        print(f"Error in future for key {key}: {e}")
                        acc_list_dic[key] = []
        ...
        for key in key_list:
            if key in acc_list_dic:
                ...
                accuracy = np.mean(np.array(acc_list_dic[key]))
                acc_str = f"{accuracy:.1%}"
                subtask_acc_list[key] = acc_str
                
        fitness_str = cal_acc(acc_list)
        sol['test_fitness'] = fitness_str
        sol['subtask_acc_list'] = subtask_acc_list
...
if __name__ == "__main__":
    ...
    parser.add_argument('--expr_name', type=str, default="math_gpt4omini_results")
    ...
    parser.add_argument('--train_dir_path', type=str, default="dataset/math/validate")
    parser.add_argument('--test_dir_path', type=str, default="dataset/math/test")
    # test_math_486
    ...
    parser.add_argument('--update_model',
                        type=str,
                        default='gpt-4.1',
                        choices=['gpt-4.1', 'gpt-4.1-20250414', 'gpt-3.5-turbo-0125', 'gpt-4o-2024-05-13', 'gpt-4o-2024-08-06', 'gpt-4o-mini-2024-07-18', 'gpt-4o-mini-20240718', 'gpt-4o-mini'])
    parser.add_argument('--test_adaption', action='store_true', default=False)
