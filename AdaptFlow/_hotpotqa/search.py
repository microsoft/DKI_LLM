...
from hotpotqa_prompt import get_init_archive, get_prompt, get_meta_prompt, get_adaptation_prompt, get_reflexion_prompt, get_fail_reflexion_prompt

from utils import load_all_examples, calculate_score, random_id, bootstrap_confidence_interval, extract_median, get_context_str
...
def process_key_evaluation(args, key, sol, adaptation_archive, acc_list_dic, adaptation_file_path):
    ...
    try:
        if sol['generation'] == "initial":
            ...
            # 随机采样六道题
            task_dsc = random.sample(load_all_examples(args.test_dir_path)[key], 2)
            task_dsc = "\n".join([
                f"##The task case {i+1} is following:\n{task['question']}\nNote that you are given context:\n{get_context_str(task)}\n\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--expr_name', type=str, default="hotpotqa_gpt4omini_results")
    ...
    parser.add_argument('--train_dir_path', type=str, default="dataset/hotpotqa/validate")
    parser.add_argument('--test_dir_path', type=str, default="dataset/hotpotqa/test")
    # test_math_486
